...
- (instancetype)initWithFrame:(CGRect)frame rectFrame:(CGRect)rectFrame rectColor:(UIColor *)rectColor {
    ...
    if (self) {
        ...
        if (CGColorEqualToColor(rectColor.CGColor, [UIColor clearColor].CGColor)) {
            //rectColor = HexUIColorFromRGB(0x01A8AE, 1);//[UIColor whiteColor];
            rectColor = RGBA(1, 0xa8, 0xae, 1);
        }
        ...
        [_torchSwithButton setTitle:LocalForkey(@"轻触照亮") forState:UIControlStateNormal];
        [_torchSwithButton setTitle:LocalForkey(@"轻触关闭") forState:UIControlStateSelected];
        ...
        _tipsLabel.text = LocalForkey(@"将二维码放入框内即可扫描");
